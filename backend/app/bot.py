...
@tree.command(name="watchlist", description="Show your stock watchlist with current prices", guild=_guild)
async def cmd_watchlist(interaction: discord.Interaction):
    ...
    try:
        ...
        if not items:
            await interaction.followup.send("Your watchlist is empty. Use `/add` to add stocks.")
            return
        ...
        for item in items:
            try:
                ...
                change = f" ({'+' if pct >= 0 else ''}{pct:.2f}%)" if pct is not None else ""
            except Exception:
                price, change = "N/A", ""
            ...
            for rule in item.rules:
                ...
                lines.append(f"  `#{rule.id}` {rule.rule_type.replace('_', ' ')} {direction} {val} · {_fmt_cooldown(rule.cooldown_seconds)} cooldown")
...
async def cmd_add(interaction: discord.Interaction, symbol: str):
    ...
    db = SessionLocal()
    try:
        item = await asyncio.to_thread(add_stock_to_watchlist, db, BOT_USER_ID, symbol.upper())
        await interaction.followup.send(f"✅ **{item.symbol}** added to your watchlist.")
...
@tree.command(name="remove", description="Remove a stock from your watchlist", guild=_guild)
@app_commands.describe(symbol="Ticker symbol e.g. AAPL")
async def cmd_remove(interaction: discord.Interaction, symbol: str):
    await interaction.response.defer()
    db = SessionLocal()
    try:
        removed = await asyncio.to_thread(remove_stock_from_watchlist, db, BOT_USER_ID, symbol.upper())
        if removed:
            await interaction.followup.send(f"✅ **{symbol.upper()}** removed from your watchlist.")
        else:
            await interaction.followup.send(f"❌ **{symbol.upper()}** is not in your watchlist.")
    except ValueError as e:
        await interaction.followup.send(f"❌ {e}")
    finally:
        db.close()
...
async def cmd_rules(interaction: discord.Interaction, symbol: str):
    ...
    db = SessionLocal()
    try:
        rules = await asyncio.to_thread(get_stock_rules, db, BOT_USER_ID, symbol.upper())
        if not rules:
            await interaction.followup.send(f"No rules set for **{symbol.upper()}**. Use `/add-rule` to create one.")
            return
        ...
        embed = discord.Embed(title=f"🔔 Rules for {symbol.upper()}", description="\n".join(lines), color=0x3498DB)
        await interaction.followup.send(embed=embed)
    except ValueError as e:
        await interaction.followup.send(f"❌ {e}")
    finally:
...
@tree.command(name="add-rule", description="Add an alert rule for a stock", guild=_guild)
...
async def cmd_add_rule(
    ...
    db = SessionLocal()
    try:
        target = -abs(value) if rule_type == "percent_change" and direction == "below" else abs(value)
        rule = await asyncio.to_thread(
            add_rule_to_stock, db, BOT_USER_ID, symbol.upper(),
            rule_type, direction, target, cooldown,
        )
...
@tree.command(name="delete-rule", description="Delete an alert rule by ID", guild=_guild)
...
@tree.command(name="check", description="Manually trigger an alert check for all your rules", guild=_guild)
async def cmd_check(interaction: discord.Interaction):
    ...
    try:
        ...
        not_met     = [r for r in results if not r.get("triggered") and not r.get("on_cooldown")]

        lines = []
        for r in triggered:
            lines.append(f"🔴 **{r['symbol']}** rule `#{r['rule_id']}` triggered — actual: {r['actual_value']:.4f}")
        for r in not_met:
            lines.append(f"⚪ **{r['symbol']}** rule `#{r['rule_id']}` not met — actual: {r['actual_value']:.4f}")
        for r in on_cooldown:
            lines.append(f"⏳ **{r['symbol']}** rule `#{r['rule_id']}` on cooldown — {r['cooldown_remaining_seconds']}s left")
